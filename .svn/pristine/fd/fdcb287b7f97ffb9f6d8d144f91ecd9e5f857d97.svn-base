...
public class OrderActivity extends Activity {
	private Button home_back,save,btn_oldOrder;
...
//	private orderAdapter adapter;
	...
	private void initData() {
		...
		if (bun != null) {
			...
			order = (Order) bun.get("order");
		}
		
		if(order!=null){//修改
			/**查明细记录*/	
			list = OrderDetail.findByCustId(order.getCustId(),order.getUserId(),
...
//		 adapter = new orderAdapter();
		orderExpandableListAdapter=new OrderExpandableListAdapter
				(OrderActivity.this,new ArrayList<List<OrderDetail>>(),new ArrayList<OrderDetail>());
	...
	private OnClickListener BtnClicked = new OnClickListener() {
		...
		public void onClick(View v) {
			...
			case R.id.add_gift://赠品
				...
				break;
			case R.id.oldOrder:
				Intent	i1 = new Intent(OrderActivity .this, OldOrderTotalActivity.class);
	...
	private final OnClickListener unitListener = new OnClickListener() {
		public void onClick(View v) {
			...
			case R.id.unit:
				...
				adapter1 = new SimpleAdapter(OrderActivity.this, unitList,
...
//							}
							...
							OrderDetail orderDetail = new OrderDetail
								...
								boolean flag  = true;
								//添加品项去重复
								for ( BaseOrderDetail boDetail : list) {
...
/***********订单adapter*****************************/
	public class OrderExpandableListAdapter extends BaseExpandableListAdapter {
		public List<List<OrderDetail>> childArray =new ArrayList<List<OrderDetail>>();
		public List<OrderDetail> groupArray = new ArrayList<OrderDetail>();
		private LayoutInflater childInflater;
		private LayoutInflater groupInflater;
		private Activity activity;
		public OrderExpandableListAdapter(Activity activity,List<List<OrderDetail>> childArray ,List<OrderDetail> groupArray) {
					...
					overOk2.setOnClickListener(new OnClickListener() {
						...
						public void onClick(View v) {
							for (int i = 0; i < list.size(); i++) {
								...
								for (OrderDetail or : childArray.get(n)) {
...
//	}
	...
	private void commit() {
		View overdiaView1 = View.inflate(OrderActivity.this,
				R.layout.dialog_order_confirmation, null);
		final Dialog overdialog1 = new Dialog(OrderActivity.this,
				R.style.dialog_xw);
...
//		TextView02.setText("确认提交订单？");
	...
	private boolean saveDetailcache() {
		...
		try {
			...
			if (order == null) {
				if (orderNew==null) {
					 orderNew = new Order(shop.getCustId(),
...
//				}
	...
	private void getTotal(){
		...
		for(BaseOrderDetail order : list){
...
//		totalPremiums.setText(premiumsQuntity+ "箱");
	...
	private void notifyList(){
		List<List<OrderDetail>> childArray = new ArrayList<List<OrderDetail>>();
		List<OrderDetail> groupArray = new ArrayList<OrderDetail>();
		List<OrderDetail> child = new ArrayList<OrderDetail>();
		for (OrderDetail orderDetail : list) {
		...
		for (OrderDetail od : groupArray) {
			child = new ArrayList<OrderDetail>();
