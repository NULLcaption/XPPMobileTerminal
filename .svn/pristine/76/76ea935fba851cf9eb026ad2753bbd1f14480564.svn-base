...
//	private Dialog overdialog;
	public SkuAdapter(Activity activity,List<BaseParameter> bpList, Product product) {
...
//	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		String[] unitItems=product.getSkuUnit().split(",");
...
//		});
		...
		if("9999".equals(bp.getObj1())){
			holder.tv_date.setText("其他");
		}else{
			holder.tv_date.setText(bp.getObj1()+"年"+bp.getObj2()+"月");
		...
		if(bp.getObj3()==null){
			...
			for (int i = 0; i < unitItems.length; i++) {
				if(bp.getObj3().equals(unitItems[i])){
					holder.sp_unit.setSelection(i);
				}
			}
		}
